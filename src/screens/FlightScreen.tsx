...
async function scheduleShiftNotifications(
  ...
  // Optimization: use Promise.all to map notifications in parallel instead of sequentially
  const promises = shiftFlights.map(async (item) => {
    ...
    try {
      ...
      console.warn('[scheduleShiftNotifications] error scheduling notification for', flightNumber, e);
      return null;
    }
  });

  const ids = await Promise.all(promises);
  newIds.push(...ids.filter((id): id is string => id !== null));
...
async function schedulePinnedNotifications(item: any, tab: 'arrivals' | 'departures'): Promise<void> {
  ...
  if (tab === 'arrivals') {
    ...
    // Optimization: use Promise.all to schedule pinned notifications in parallel
    const promises = phases.map(async (phase) => {
      ...
      try {
        ...
        console.warn('[schedulePinnedNotifications] error scheduling for phase', phase.title, e);
        return null;
      }
    });
