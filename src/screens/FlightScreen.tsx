...
import {
  ...
  Animated, PanResponder, NativeModules, Platform, AccessibilityActionEvent,
} from 'react-native';
...
export default function FlightScreen() {
  ...
  const renderFlight = useCallback(({ item }: { item: any }) => {
    ...
    const accLabel = `${pinnedState}${flightNumber}, ${airline}, ${direction} ${originDest}, ${time}, ${statusText}`;
      ...
      <SwipeableFlightCard
        ...
        onAccessibilityAction={(event: AccessibilityActionEvent) => {
          if (event.nativeEvent.actionName === 'toggle_pin') {
            if (isPinned) unpinFlight(); else pinFlight(item);
          }
        }}
      >
